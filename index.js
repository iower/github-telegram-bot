...
const process = async () => {
  try {
    ...
    if (fetchedCommits?.message?.includes('limit exceeded')) {
      console.error((new Date()).toISOString(), fetchedCommits.message);
      return;
    }
    ...
    try {
      ...
      console.info((new Date()).toISOString(), `Will be created: ${savedCommitsFilePath}`);
    ...
    if (newCommits.length) {
      console.log((new Date()).toISOString(), `New commits! (${newCommits.length})`);
    ...
    console.error((new Date()).toISOString(), 'process err |', err);
  }
}
...
const saveCommits = (commits) => {
  console.log((new Date()).toISOString(), `Save commits to ${savedCommitsFilePath}`);
  fs.writeFileSync(savedCommitsFilePath, JSON.stringify(commits), (e) => {
    if (e) {
      console.error((new Date()).toISOString(), `Cannot write ${savedCommitsFilePath}`);
      throw new Error(e)
    }
  });
}
